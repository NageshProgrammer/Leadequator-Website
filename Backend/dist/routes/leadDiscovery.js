...
import { buyerKeywords, redditPosts, quoraPosts, usersTable, quoraAiReplies, redditAiReplies } from "../config/schema.js";
...
/* ===============================
   RUN REDDIT SCRAPING
================================ */
router.post("/reddit/run", async (req, res) => {
    try {
        ...
        if (!aiResponse.ok)
            console.error("Reddit scraper service reported an issue.");
...
/* ===============================
   RUN QUORA SCRAPING
================================ */
router.post("/quora/run", async (req, res) => {
    try {
        ...
        if (!aiResponse.ok)
            console.error("Quora scraper service reported an issue.");
        // 5. Count New Posts & Deduct Credits
        const [countResult] = await db
            .select({ count: sql `cast(count(*) as int)` })
            .from(quoraPosts)
...
/* ===============================
   FETCH QUORA POSTS
================================ */
router.get("/quora/posts", async (req, res) => {
    try {
        const { userId } = req.query;
        if (!userId)
            return res.status(400).json({ error: "Missing userId" });
