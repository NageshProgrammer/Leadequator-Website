...
// ✅ Schema Imports
import { onboardingProgress, companyDetails, targetMarket, buyerKeywords, platformsToMonitor, usersTable, userSubscriptions, eventWaitlist, // ✅ Added this import
...
async function sendEventEmail(userEmail, userName) {
    try {
...
cron.schedule("0 0 * * *", async () => {
    ...
    try {
        ...
        await db.update(userSubscriptions)
            .set({ status: "EXPIRED" })
            .where(and(eq(userSubscriptions.status, "ACTIVE"), lte(userSubscriptions.endDate, now)));
...
app.post("/api/onboarding", async (req, res) => {
    try {
        ...
        if (keywordsData?.keywords?.length > 0) {
            await db.insert(buyerKeywords).values(keywordsData.keywords.map((k) => ({ userId, keyword: k })));
        }
    ...
    try {
        ...
        if (Array.isArray(keywords)) {
            ...
            if (keywords.length > 0) {
                await db.insert(buyerKeywords).values(keywords.map((k) => ({ userId, keyword: k })));
            }
        }
        res.json({ success: true });
    }
