...
import { Button } from "@/components/ui/button";
import { KPICard } from "@/components/dashboard/KPICard";
import { MessageSquare, AlertCircle, ThumbsUp, Users, TrendingUp, Activity } from "lucide-react";
...
const DashboardOverview = () => {
  const navigate = useNavigate();
  const { threads, refreshLeads } = useLeads();
  ...
  const highIntentLeads = threads.filter(t => t.intent >= 70);
  const repliesSent = threads.filter(t => t.replyStatus === "Sent").length;
  const linkClicks = totalLeads * 3; // Placeholder logic
  const conversionRate = totalLeads > 0 ? ((repliesSent / totalLeads) * 100).toFixed(1) + "%" : "0%";

  const kpiData = [
    { icon: MessageSquare, label: "New Comments", value: totalLeads.toString() },
    { icon: AlertCircle, label: "High-Intent (≥70)", value: highIntentLeads.length.toString() },
    { icon: ThumbsUp, label: "Auto Replies Sent", value: repliesSent.toString() },
    { icon: Users, label: "Link Clicks", value: linkClicks.toString() },
    { icon: TrendingUp, label: "Engage→Lead %", value: conversionRate },
    { icon: Activity, label: "Avg Reply Time", value: "—" },
  ];

  const sentimentData = [
    { name: "Positive", value: threads.filter(t => t.sentiment === "Positive").length, color: "#22c55e" },
    { name: "Neutral", value: threads.filter(t => t.sentiment === "Neutral").length, color: "#94a3b8" },
    { name: "Negative", value: threads.filter(t => t.sentiment === "Negative").length, color: "#ef4444" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="p-6">
          ...
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={sentimentData} dataKey="value" innerRadius={60} outerRadius={80}>
        ...
        <Card className="p-6">
          <h3 className="font-bold mb-4">Recent High-Intent Leads</h3>
          <div className="space-y-4">
            {highIntentLeads.slice(0, 3).map((lead) => (
              <div key={lead.id} className="flex items-center justify-between p-4 border rounded-lg">
                <div>
                  <div className="font-bold">{lead.user}</div>
                  <div className="text-sm text-muted-foreground">via {lead.platform}</div>
                </div>
                <Badge className="bg-green-500">{lead.intent}</Badge>
                <Button size="sm" onClick={() => navigate("/monitor-stream")}>View</Button>
              </div>
            ))}
          </div>
        </Card>
