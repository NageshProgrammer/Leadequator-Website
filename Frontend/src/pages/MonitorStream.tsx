...
import { Thread, useLeads } from "./LeadContext";

const MonitorStream = () => {
  const { user } = useUser();
  const { toast } = useToast();
  const { threads, loading, refreshLeads, setThreads } = useLeads(); // Global state
  ...
  const runScraper = async () => {
    ...
    try {
      ...
      toast({ title: "Scraping Started 🚀", description: "Updating your feed shortly..." });
      
      // Auto-refresh the global state after scraper finishes
  ...
  return (
    <div className="p-4 md:p-8 bg-background min-h-screen">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Monitor <span className="text-yellow-400">Stream</span></h1>
          </div>
          <div className="flex gap-3">
            <Button onClick={runScraper} disabled={running} className="bg-yellow-400 text-black">
              {running && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Run Scraper
            </Button>
          </div>
        </div>
        ...
        <Card>
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={6} className="text-center py-10">Loading leads...</TableCell></TableRow>
              ) : (
                threads.map((t) => (
                  <TableRow key={t.id}>
                    <TableCell>{t.timestamp}</TableCell>
                    <TableCell><Badge>{t.platform}</Badge></TableCell>
                    <TableCell className="max-w-xs truncate">{t.post}</TableCell>
                    <TableCell>{t.intent}</TableCell>
                    <TableCell>{t.replyStatus}</TableCell>
                    <TableCell>
                      <Button size="sm" onClick={() => setSelected(t)}><Sparkles className="h-4 w-4" /></Button>
                    </TableCell>
                  </TableRow>
                ))
              )}
        ...
        {selected && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-[50vw] h-full bg-background p-4">
              <DetailPane 
                comment={{ ...selected, followers: 0 }} 
                onClose={() => setSelected(null)} 
                onSend={handleSendReply} 
              />
            </div>
