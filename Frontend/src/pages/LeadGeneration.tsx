...
export default function LeadGeneration() {
  ...
  // State for Discovery Parameters
  ...
  const [stats, setStats] = useState({ totalIndex: 0, highIntent: 0, cacheHits: 2, aiTokens: "4.2k" });
  ...
  // Helper to add logs
  const addLog = (message: string, type: "SYS" | "AI" | "DB" = "SYS") => {
  ...
  // 👉 2. Main Search Handler (Integrates AI)
  const handleDeepSearch = async () => {
    ...
    try {
      // Prepare the payload for your Python AI
      const payload = {
        userId: user.id,
        industry,
        geography,
      ...
      // Simulate streaming logs (optional visual effect)
      addLog("Connecting to neural search indices...", "DB");
      setTimeout(() => addLog("Parsing intent signals...", "AI"), 800);
      setTimeout(() => addLog(`Found ${data.results?.length || 0} potential matches.`, "SYS"), 1500);

      // Update Stats based on AI response
      setStats(prev => ({
        ...prev,
        totalIndex: (data.results?.length || 0) + prev.totalIndex,
        highIntent: (data.highIntentCount || 0) + prev.highIntent,
        aiTokens: "5.1k" // Update dynamically if your API returns usage
      }));

    } catch (error) {
      console.error("Search failed:", error);
      addLog("CRITICAL: Connection to AI Swarm lost.", "SYS");
    } finally {
        ...
        >
            ...
            <div className={`${glassPanelStyle} flex-grow relative overflow-hidden flex flex-col`}>
              ...
              <div className="space-y-5 flex-grow">
                <div>
                  ...
                  <Input value={industry} onChange={(e) => setIndustry(e.target.value)} placeholder="e.g. Cloud Security" className={inputStyle} />
                </div>
                <div>
                  <label className={labelStyle}>Geography</label>
                  <Input value={geography} onChange={(e) => setGeography(e.target.value)} placeholder="e.g. United States, UK" className={inputStyle} />
                </div>
                <div>
                  <label className={labelStyle}>Tracked Keywords</label>
                  <Input value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="e.g. Zero Trust, SOC, SIEM" className={inputStyle} />
                </div>
                <div>
                  <label className={labelStyle}>Business Size</label>
                  <Input value={businessSize} onChange={(e) => setBusinessSize(e.target.value)} placeholder="e.g. Medium (100-500)" className={inputStyle} />
                </div>
                <div>
                  <label className={labelStyle}>Buying Signals / Triggers</label>
                  <Textarea value={buyingSignals} onChange={(e) => setBuyingSignals(e.target.value)} placeholder="e.g. Hiring security leads..." className={`${inputStyle} min-h-[100px] resize-none pt-4`} />
                </div>
              </div>
          ...
          <div className="lg:col-span-8 flex flex-col gap-6 lg:gap-8">
            ...
            <div className={`${glassPanelStyle} flex-grow flex flex-col`}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Badge variant="outline" className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 shadow-none font-mono text-[10px] tracking-widest uppercase"><Database className="w-3 h-3 mr-1.5 inline" /> DB OK</Badge>
                  ...
                  <Button size="sm" variant="destructive" onClick={() => setLogs([])} className="h-8 text-xs font-bold bg-red-500/10 text-red-500 hover:bg-red-500/20 border border-red-500/20 rounded-lg">
                    <Trash2 className="h-3.5 w-3.5 mr-1.5" /> PURGE
                  </Button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 lg:gap-6">
              <div className="bg-[#050505]/60 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-5 shadow-lg flex flex-col justify-center items-center text-center">
                <p className="text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest mb-1">Total Index</p>
                <p className="text-3xl font-black text-white">{stats.totalIndex}</p>
              </div>
              <div className="bg-[#050505]/60 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-5 shadow-lg flex flex-col justify-center items-center text-center">
                ...
                <p className="text-3xl font-black text-white">{stats.highIntent}</p>
              </div>
              <div className="bg-[#050505]/60 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-5 shadow-lg flex flex-col justify-center items-center text-center">
                <p className="text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest mb-1">Cache Hits</p>
                <p className="text-3xl font-black text-[#fbbf24]">{stats.cacheHits}</p>
              </div>
              <div className="bg-[#050505]/60 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-5 shadow-lg flex flex-col justify-center items-center text-center">
                <p className="text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest mb-1">AI Tokens</p>
                <p className="text-3xl font-black text-emerald-400">{stats.aiTokens}</p>
              </div>
            </div>
